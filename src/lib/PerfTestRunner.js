...
PerfTestRunner.logInfo = function (text) {
	if (!window.testRunner) this.log(text);
};
...
PerfTestRunner.logStatistics = function (values, unit, title) {
	...
	this.log('');
	this.log(title);
	if (statistics.values) this.log('values ' + statistics.values.join(', ') + ' ' + statistics.unit);
...
PerfTestRunner.gc = function () {
	if (window.GCController) window.GCController.collectAll();
	else {
		function gcRec(n) {
			if (n < 1) return {};
...
PerfTestRunner.log = function (text) {
	if (logLines) logLines.push(text);
	else logInDocument(text);
};
...
PerfTestRunner.formatException = function (text, exception) {
	return 'Got an exception while ' + text + ' with name=' + exception.name + ', message=' + exception.message + '\n' + exception.stack;
};
...
function start(test, scheduler, runner) {
	...
	if (test.traceEventsToMeasure && !test.tracingCategories) {
		PerfTestRunner.logFatalError("test's traceEventsToMeasure is " + "specified but test's tracingCategories is empty");
		return;
	}
	iterationCount = test.iterationCount || (window.testRunner ? 5 : 20);
	if (test.warmUpCount && test.warmUpCount > 0) completedIterations = -test.warmUpCount;
	...
	if (test.doNotIgnoreInitialRun) completedIterations++;
	...
	if (test.tracingCategories) {
		PerfTestRunner.log(
			'Tracing based metrics are specified but ' +
...
function scheduleNextRun(scheduler, runner) {
	...
	scheduler(function () {
		...
		if (completedIterations == 0) console.time('blink_perf');
		...
		if (completedIterations < iterationCount) scheduleNextRun(scheduler, runner);
		else finish();
	});
}
...
function ignoreWarmUpAndLog(measuredValue) {
	...
	// Tests that run in workers are not impacted by the iteration control.
	if (!currentTest.runInWorker && completedIterations <= 0) PerfTestRunner.log('Ignoring warm-up run (' + labeledResult + ')');
	else {
...
function finish() {
	...
	if (window.testRunner) {
		if (currentTest.traceEventsToMeasure && testRunner.telemetryIsRunning) {
			testRunner.stopTracingAndMeasure(currentTest.traceEventsToMeasure, function () {
...
PerfTestRunner.measureValueAsync = function (measuredValue) {
	...
	if (completedIterations >= iterationCount) finish();
};

PerfTestRunner.addRunTestStartMarker = function () {
	if (!window.testRunner || !window.testRunner.telemetryIsRunning) return;
	if (completedIterations < 0) console.time('blink_perf.runTest.warmup');
	else console.time('blink_perf.runTest');
};

PerfTestRunner.addRunTestEndMarker = function () {
	if (!window.testRunner || !window.testRunner.telemetryIsRunning) return;
	if (completedIterations < 0) console.timeEnd('blink_perf.runTest.warmup');
	else console.timeEnd('blink_perf.runTest');
};
...
function measureFrameTimeOnce() {
	if (lastFrameTime != -1) PerfTestRunner.addRunTestEndMarker();
	...
	if (returnValue - 0 === returnValue) {
		if (returnValue < 0) PerfTestRunner.log('runFunction returned a negative value: ' + returnValue);
		return returnValue;
	}
...
function measureRunsPerSecondOnce() {
	...
	while (totalTime < timeToRun) {
		...
		if (completedIterations < 0 && totalTime < 100) callsPerIteration = Math.max(10, 2 * callsPerIteration);
	}
...
function callRunAndMeasureTime(callsPerIteration) {
	...
	for (var i = 0; i < callsPerIteration; i++) currentTest.run();
...
PerfTestRunner.measurePageLoadTime = function (test) {
	...
	test.run = function () {
		if (!test.chunkSize) this.chunkSize = 50000;
